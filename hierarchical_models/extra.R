...
myinits <- list(
  list(alphamu=.5, alphalambda=1, 
       alpha=rep(.5, ns), beta=.1))

parameters <- c("alpha", "alphamu", "alphasigma", "beta", "predk")  # Parameters to be monitored

# For a detailed description type "?rstan".
samples <- stan("test.stan",   
                data=data, 
...
par(mar=c(2,2,1,0))
plot(alpha1[keep],beta[keep], xlab="", ylab="", xlim=c(0,1), ylim=c(0,1),
     axes=F)
points(alpha2[keep],beta[keep], col="red")
points(alpha3[keep],beta[keep], col="green")
points(alpha4[keep],beta[keep], col="blue")
box(lty=1)

par(mar=c(2,1,1,4))
plot(d.beta$y, d.beta$x, ylim=range(c(0,1)), xlim=c(12,0),type='l', axes=F, 
     xlab="", ylab="")
#plot(d.beta1$y, d.beta1$x, ylim=range(c(0,1)), xlim=rev(range(d.beta1$y)),
#     type='l', axes=F, xlab="", ylab="")
lines(d.beta$y, d.beta$x, col="red")
lines(d.beta$y, d.beta$x, col="green")
lines(d.beta$y, d.beta$x, col="blue")
axis(4, at=c(0,1))
...
hist(parameters$alphamu)
hist(parameters$alphasigma)
hist(parameters$alpha)
hist(parameters$beta)

#95% HDI
HDIofMCMC(parameters$alpha, credMass = 0.95)
HDIofMCMC(parameters$alphamu, credMass = 0.95)
HDIofMCMC(parameters$alphasigma, credMass = 0.95)
HDIofMCMC(parameters$beta, credMass = 0.95)
HDIofMCMC(alpha1, credMass = 0.95)
HDIofMCMC(alpha2, credMass = 0.95)
HDIofMCMC(alpha3, credMass = 0.95)
HDIofMCMC(alpha4, credMass = 0.95)
